    ...
    http.onreadystatechange = function () {
        ...
        if (http.readyState == 4 && http.status == 200) {
            ...
            if(user.verify = true){
               ...
               alert("Login Incorrecto. Revisa el nombre de usuario y/o la contraseña!");
            }
...
function deleteUser(){
    ...
    http.open("POST", "http:/localhost:8080/Casino/DeleteUser", true);
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    http.send("dni=" + document.getElementById("deleteId").value + "&&password=" + document.getElementById("deletePassword").value);
